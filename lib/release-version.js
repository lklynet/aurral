...
const RELEASE_NOTE_ITEM_RE = /^(?:[-*+]\s+|\d+[.)]\s+)/;
...
export function extractReleaseNoteItems(body, maxItems = 8) {
  return String(body || "")
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => RELEASE_NOTE_ITEM_RE.test(line))
    .map((line) =>
      line
        .replace(RELEASE_NOTE_ITEM_RE, "")
        .replace(/\[([^\]]+)\]\([^)]*\)/g, "$1")
        .replace(/\s+https?:\/\/\S+/g, "")
        .replace(/\s+in\s*$/i, "")
        .replace(/\s+/g, " ")
