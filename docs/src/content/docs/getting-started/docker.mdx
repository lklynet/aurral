...
1. Generate and save a persistent `BETTER_AUTH_SECRET`, for example with `openssl rand -hex 32`.
2. Set `BETTER_AUTH_URL` to the URL that users use to reach Aurral.
3. Open `http://localhost:3001`.
4. Create the admin account with a name, email address, and password.
5. Connect Lidarr.
6. Set the Downloads Folder path and run the storage checks.
