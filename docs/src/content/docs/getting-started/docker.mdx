...
1. Open `http://localhost:3001`. Aurral generates and stores its Better Auth secret in `/config` and derives its auth URL from the incoming request.
2. Create the admin account with a username and password.
3. Connect Lidarr.
