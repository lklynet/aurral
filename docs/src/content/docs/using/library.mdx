...
For an available track downloaded by Aurral, right-click and choose
**Re-search** to look for another recording. The current file stays available
until Aurral finds and validates a replacement. Tracks from Lidarr do not have
this action.
...
watcher queues a targeted local scan for the changed file or folder. Unchanged
files are skipped using their stored size and modification time. The watcher
waits briefly for a file move or rename to finish and ignores `_playlists`,
`_staging`, `_fallback`, hidden folders, `_flows`, and `aurral-weekly-flow`.
