...
Spotify returns playlist items in pages of up to 50. Aurral follows every page, so very large playlists can take longer to sync. If Spotify reports more items than Aurral received, the sync fails without replacing the saved Aurral tracklist. Spotify only provides playlist item details for playlists you own or collaborate on.
...
- To replace an available Aurral-downloaded recording, right-click it and
  select **Re-search**. Aurral keeps the current file until a replacement
  passes validation.
- Only flows can automatically replace a failed download with a different track.
