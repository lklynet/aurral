...
history**. Turn it off when playback from that playlist should not affect future
Discover recommendations or connected scrobblers. The setting is independent
for each playlist and does not remove plays already recorded.
...
To retry a failed track, right-click it and select **Re-search**.
To replace an available Aurral-downloaded recording, right-click it and select
**Re-search**. Aurral keeps the current file until a replacement passes
validation.

## Remove a playlist or track

Playlist deletion and track removal cancel the Aurral pipeline before they clear the download job. The delete action can stay queued while Aurral cleans the playback playlist and files.

When you delete a playlist, Aurral removes completed files it downloaded for that playlist if no other job or playback playlist needs them. It keeps files that are still in use.

If a provider finishes a request after removal, Aurral does not import that result into the removed playlist. A provider may still show its own remote work until that provider reports the cancellation or completion.

If a download provider is no longer configured while it has tracked work, playlist removal fails and keeps the playlist and jobs. Reconfigure the provider, then retry the removal.

Aurral leaves completed Usenet and deemix source files in place when it cannot prove that it owns them. If the provider finishes after removal, check its completed-download folder for files that you want to remove.

When you create a playlist again with the same ID, old download work remains tied to the removed generation and cannot attach to the new playlist.
