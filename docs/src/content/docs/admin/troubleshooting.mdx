...
If a download client still shows work after Aurral removes the playlist, check that client's queue. Aurral blocks the removed job from creating a playlist file. If Aurral cannot remove the item from the client, the removal fails and Aurral retries it.
