...
Playlist and flow deletion run in the background. The page reports **removal queued** until the operation worker finishes.

If a download client still shows work after Aurral removes the playlist, check that client's queue. Aurral blocks the removed job from creating a playlist file. NZBGet has no verified queue-cancel operation in the current adapter, so NZBGet may finish or remove the remote request on its own.
...
Aurral stores image links in SQLite separately from the native-library image bytes. When the byte cache evicts an image, Aurral can fetch it again without another metadata lookup.
