...
- Back up the Compose file and the environment or secret store that contains your host paths and `BETTER_AUTH_SECRET`.
...
If you roll back after the migration starts, stop Aurral and restore the pre-upgrade `/config` copy and its matching `BETTER_AUTH_SECRET` before starting the previous version. Do not run the previous version against the migrated database. Keep the media mount unchanged.
