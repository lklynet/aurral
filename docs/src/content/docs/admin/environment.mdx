...
| Variable | Purpose |
| -------- | ------- |
| `AURRAL_SQLITE_CACHE_MB` | SQLite page cache for the main connection, in megabytes. Default `256`. Worker connections use a quarter of this value. Lower it on hosts with little RAM. |
| `AURRAL_SQLITE_MMAP_MB` | Size of the memory-mapped window over the database file, in megabytes. Default `1024`. This is address space, not allocated RAM; the OS shares the pages with its file cache. Set `0` to disable. |
