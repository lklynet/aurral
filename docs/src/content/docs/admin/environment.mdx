...
| `OIDC_CLIENT_SECRET` | OIDC client secret. Not required when `OIDC_TOKEN_ENDPOINT_AUTH_METHOD` is `none`. |
| `OIDC_TOKEN_ENDPOINT_AUTH_METHOD` | How Aurral authenticates to the token endpoint: `client_secret_basic` (default), `client_secret_post`, or `none`. Must match the method registered with your IdP. |
...
The Docker image uses conservative defaults for native artwork processing. These variables let you trade memory for artwork-generation throughput.

| Variable | Purpose |
| -------- | ------- |
| `AURRAL_DISCOVERY_ARTWORK_CONCURRENCY` | Number of discovery playlist covers rendered in parallel. Default `1`; accepted range `1`–`3`. |
| `AURRAL_SHARP_CONCURRENCY` | Number of libvips worker threads used by Sharp. Default `2`; accepted range `1`–`4`. |
| `MALLOC_CONF` | jemalloc page-decay settings. The Docker image defaults to `background_thread:true,dirty_decay_ms:1000,muzzy_decay_ms:1000` so freed native image buffers return to the operating system sooner, including while the application is idle. |
