...
can add and acquire artists and albums in its own root without Lidarr.
...
An add or request can choose Aurral or Lidarr when both managers are
available. Without an explicit choice, Aurral uses the saved library-owner
preference and falls back to Lidarr when it is connected.
...
Lidarr-owned artist and album changes go through Lidarr. Aurral-owned changes
write to the configured Aurral root. Aurral does not write into the Lidarr
root music library.
