...
| `PUT` | `/api/library/albums/aurral/:canonicalId` | Monitor or stop monitoring an Aurral-managed album with `{ "monitored": true \| false }` |
...
| `GET` | `/api/library/downloads/status` | Get statuses for the required `albumIds` query; use `aurral:<canonicalId>` for Aurral-managed albums |
...
Aurral album routes take the canonical album ID, not a Lidarr album ID. They
...
`PUT /api/library/artists/:mbid` sets monitoring for an Aurral-managed artist
with `monitorOption` set to `none`, `all`, `missing`, `latest`, `first`, or
`future`. `existing` returns `400` with `unsupported_monitor_mode`. The
response includes `monitoring.releaseGroupIds` for the albums being queued and
`monitoring.skipped` with a reason for each skipped album. If artist metadata is
unavailable, the route returns `503` with `metadata_unavailable` and keeps the
previous mode.
...
| `PUT` | `/api/playlists/shared-playlists/:playlistId/record-history` | Enable or disable listening history for a shared playlist |
