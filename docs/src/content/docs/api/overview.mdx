...
Better Auth owns local sign-in, sessions, and bearer tokens. Sign in with a
...
curl --fail --include \
  ...
  https://aurral.example.com/api/auth/sign-in/username
```

See [Users and authentication](/admin/users/) for OIDC, reverse-proxy, and
session details.
